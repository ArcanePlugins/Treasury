...
public class EconomyHelpSub implements Subcommand {
    ...
    @Override
    public void execute(@NotNull final CommandSource source, @NotNull final String label, final @NotNull String[] args) {
        ...
        if (args.length != 0) {
            source.sendMessage(
                    Message.of(MessageKey.ECONOMY_HELP_INVALID_USAGE, MessagePlaceholder.placeholder("label", label))
            );
            return;
        }
